...
*   `data-u-sprite="spriteKey"`：可选。给图片、按钮背景、卡片背景等声明一个 Sprite 映射关键字，Unity 端会优先按这个 key 查找配置中的图片资源。

### 建议补充的样式信息
为了提升 Unity 端还原度，建议在 `style` 中尽量明确写出以下属性：
*   `background-color`
*   `color`
*   `font-size`
*   `font-family`
...
<div data-u-type="div" data-u-name="settingsWindow" style="width: {WIDTH}px; height: {HEIGHT}px; background-color: #2c3e50;">
    ...
    <!-- 带 spriteKey 的图片占位 -->
    <div data-u-type="image" data-u-name="heroAvatarImg" data-u-sprite="heroAvatar" style="width: 96px; height: 96px; border-radius: 16px; border: 2px solid #ffffff;"></div>
